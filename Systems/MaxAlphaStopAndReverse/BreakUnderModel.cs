...
public class BreakUnderModel
{
	...
	public bool calcShort (BarData[] LookBackData , double body, double AdxValue) {
		...
		//if ((LookBackData[1].BarStartTime.Hour < 10 && LookBackData[1].BarStartTime.Minute < 31) && (LookBackData[0].Close < LookBackData[0].Open)) {
        //    firstbar = true;
		//}
	...
	public bool calcTimeup (BarData[] LookBackData , double body, SymbolScriptBase sb ) {
		...
		// time is up
		if (LookBackData[0].BarStartTime.Hour >= 15 ) {
            timeup = true;
		}
