...
public class MySystem : MySystemBase
{
	...
	public void saveOutput() {
		...
		string path1 = Path.Combine("C:\\dec\\RightEdge\\Systems\\MaxAlphaStopAndReverse","output.html");
		sw = new StreamWriter(path1);
	
		string path2 = Path.Combine("C:\\dec\\RightEdge\\Systems\\MaxAlphaStopAndReverse","output.txt");
		...
		{
			// Buys
			foreach(Position p in SymbolScript.ClosedPositions)
			{
...
public class MySymbolScript : MySymbolScriptBase
{
	...
	public override void NewBar()
	{
		...
		// Set Trail Short
		if (Bars.Current.Close < trailShortPrice - 0.35) {
		...
		if (OpenPositions.Count == 0)
		{
			...
			OutputMessage("Checking ");
			
			OutputMessage("Open Positions [" + OpenPositions.Count + "]");
			
			// Std
			if ( BU.calcShort(LookBackDataBuy, body, AdxValue) == true )
			{
				OutputMessage("CALC Short is True: " + Bars.Current.BarStartTime.ToString());
			...
			bool cover = BU.calcCover(LookBackDataSell, body, trailShortPrice, this);
			
			bool timeupShort = BU.calcTimeup(LookBackDataSell, body, this);
			...
			if ( cover == true  )
			{
				...
				// double check buy criteria
				if ( BO.calcFlip(LookBackDataBuy, body, AdxValue) == true )
				{
				
					OutputMessage("Flipp Check");
					
					// Now go long !!!
					if (OpenPositions.Count == 1 && OpenPositions.SingleOrDefault().Type == PositionType.Short )
					{
						OutputMessage("Flipping");
