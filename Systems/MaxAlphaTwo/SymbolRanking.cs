...
public class MySymbolScript : MySymbolScriptBase
{
    ...
    public override void Startup()
	{
		...
		OutputMessage("START UP {} [" + ADX_PARM + "]");
	...
	public override void NewBar()
	{
		...
		OutputMessage("New Bar 1 [" + Bars.Count + "] [" + lookBackBuy + "]");
		...
		{
			OutputMessage("Open Positions [" + OpenPositions.Count + "] [" + RankValue + "] [" + Rank + "] [" + AdxValue + "] [" + BO.calcShortInitial(LookBackDataBuy, body, AdxValue) + "]");

			// Std
			if ( BO.calcShortInitial(LookBackDataBuy, body, AdxValue) == true && Rank ==0)
			{
			...
			if ( BO.calcShort(LookBackDataBuy, body, AdxValue) == true )
			{
				...
				if ( OpenPositions.Count == 0 && tradedTodayShort == false && TradingSystem.systemTradedToday == false )
				{
					OutputMessage("Shorting by calc Body [" + body + "] [" + AdxValue + "]");
			...
			bool cover = BO.calcCover(LookBackDataBuy, body, trailShortPrice, VwapValue, this);
